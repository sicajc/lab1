...
void pipe_cycle()
{
    // This executes a cycle, and calls all the stages.
#ifdef DEBUG
    printf("\n\n----\n\nPIPELINE:\n");
    printf("DCODE: ");
...
void pipe_stage_wb()
{
    ...
    // Note WB does not need to stall at all
    if (!pipe.wb_op)
        return;
...
void pipe_stage_mem()
{
    ...
    /* Otherwise simply grabs the op out of our input slot */
    // Simulating the input register
    ...
    // Store operations, write back to the cache and mark the cache dirty bit as 1
    case OP_SB:
        switch (op->mem_addr & 3)
        {
        case 0:
...
void pipe_stage_execute()
{
    ...
    // Adds a guard at the beginning of the function, simply returns the function is alright.
    ...
    // Pipeline handles branch recovery at execute stage, stall for 3 cycles
    if (op->branch_taken)
...
void pipe_stage_decode()
{
    ...
    /* Using the shifter bit wise operation to get the correct bit fields       */
    ...
    // Thus using the same principle, I can find the pc's bit fields
